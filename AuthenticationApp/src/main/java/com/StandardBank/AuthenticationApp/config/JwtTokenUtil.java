package com.StandardBank.AuthenticationApp.config;
...
public class JwtTokenUtil {
    ...
    public String generateToken(String username) {
        return Jwts.builder()
                ...
                .signWith(SignatureAlgorithm.HS512, secret)  // Sign the token with HS512 algorithm and the secret key
    ...
    private Claims getAllClaimsFromToken(String token) {
        return Jwts.parser().setSigningKey(secret).parseClaimsJws(token).getBody();  // Parse the token and return its body/claims
    }
